...
export type formatDayNumber = (locale: string, date: Temporal.PlainDate) => string;
...
export type monthNames = (locale: string, style: NameStyle) => readonly string[];
...
export const resolveLocale: resolveLocale = (locale) => {
  if (locale === undefined) return new Intl.DateTimeFormat().resolvedOptions().locale;
...
const cacheKey = (locale: string, options: Intl.DateTimeFormatOptions): string => {
 ...
 */
const utcEpoch = (year: number, monthIndex: number, day: number, hour = 0, minute = 0): number => {
...
const inUtc = (options: Intl.DateTimeFormatOptions): Intl.DateTimeFormatOptions => ({
  ...options,
  timeZone: "UTC",
});

export const formatPlainDate: formatPlainDate = (locale, date, options = { dateStyle: "long" }) =>
...
export const formatPlainDateRange: formatPlainDateRange = (
  ...
  getDateTimeFormat(locale, inUtc(options)).formatRange(plainDateEpoch(start), plainDateEpoch(end));
...
export const formatYearMonth: formatYearMonth = (
  ...
  return getDateTimeFormat(locale, inUtc(options)).format(utcEpoch(iso.year, iso.month - 1, 1));
};

export const formatDayNumber: formatDayNumber = (locale, date) =>
  getDateTimeFormat(locale, inUtc({ day: "numeric" })).format(plainDateEpoch(date));

export const formatPlainTime: formatPlainTime = (
  locale,
  time,
  options = { hour: "numeric", minute: "2-digit" },
) => getDateTimeFormat(locale, inUtc(options)).format(utcEpoch(2000, 0, 1, time.hour, time.minute));

export const formatHour: formatHour = (locale, hour) =>
  getDateTimeFormat(locale, inUtc({ hour: "numeric" })).format(utcEpoch(2000, 0, 1, hour));
...
type WeekInfo = { readonly firstDay: number; readonly weekend: readonly number[] };
